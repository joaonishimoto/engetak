  ...
  return (
    <div className={`h-full w-full overflow-y-auto overflow-x-hidden scrollbar-thin ${
      ...
      <nav className="h-screen flex flex-col bg-white border-r shadow-lg">
        <div className={`flex justify-between items-center my-3 -mb-1 ${
          isExpanded ? "ml-10" : ""
        }`}>
          <Image
            src={"/engetak.png"}
            ...
            className={`overflow-hidden ${
              isExpanded ? "w-20 h-20" : "w-0"
            }`}
          />
          {isExpanded 
            ? <p className="text-app-500 font-bold mr-auto text-xl -ml-2">ENGETAK</p>
            : null}
  ...
  return (
    <Link href={path} className="">
      <li
        ...
        ${isActive
          ? "bg-app-active text-white font-semibold shadow-[2px_2px_5px_1px_rgba(0,0,0,0.2)] transition-all duration-300"
          : "text-zinc-400 hover:text-app-active hover:bg-zinc-100 hover:font-semibold transition-all duration-300"
        }
        ${isExpanded 
          ? "hover:translate-x-1" 
          : ""
      ...
      >
        ...
        <span
          className={`overflow-hidden ${
            isExpanded ? "ml-5 h-[28px]" : "w-0 h-[28px]"
          }`}
        >
          {text}
        </span>
